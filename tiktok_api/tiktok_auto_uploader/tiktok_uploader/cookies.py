...
def load_cookies_from_file(filename: str, cookies_path=None):
    if not cookies_path:
        cookie_path = os.path.join(get_cookies_default_path(), filename + ".cookie")
    else:
...
def delete_all_cookies_files(cookies_path=None):
    if not cookies_path:
        cookie_dir = os.path.join(get_cookies_default_path())
    else:
...
def get_cookies_default_path():
    current_script_path = os.path.abspath(__file__)
    tiktok_auto_uploader_path = os.path.dirname(os.path.dirname(current_script_path))
    cookies_path = os.path.join(tiktok_auto_uploader_path, Config.get().cookies_dir)
